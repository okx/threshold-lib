...
func NewP2(bobPri, E_x1 *big.Int, publicKey *ecdsa.PublicKey, paiPub *paillier.PublicKey, message string, ped *pedersen.PedersenParameters) *P2Context {
...
func (p2 *P2Context) Step2(cmtD *commitment.Witness, p1Proof *schnorr.Proof) (*zkp.AffGProof, *big.Int, error) {
	q := curve.N
	N2 := new(big.Int).Mul(p2.paiPub.N, p2.paiPub.N)
	...
	rnd := crypto.RandomNum(p2.paiPub.N)

	a_x1, _ := p2.paiPub.HomoMulPlain(p2.E_x1, a)
	a_x1_b, _ := p2.paiPub.HomoAddPlain(a_x1, b)
	E_k2_h_xr := new(big.Int).Mod(new(big.Int).Mul(a_x1_b, new(big.Int).Exp(rnd, p2.paiPub.N, N2)), N2)
	...
	aff_g_proof := zkp.PaillierAffineProve(p2.ped, st, wit)

	return aff_g_proof, E_k2_h_xr, nil
}
